...
contract DeviceBoundWallet {
    ...
    function reconnectWallet(
        ...
        // 1. Derive hash as with WebAuthn (see RFC, typically sha256 of authenticatorData || clientDataJSON)
        bytes32 clientHash = sha256(bytes.concat(authenticatorData, clientDataJSON));
        // 2. Assume the calling device is trying to prove it owns the primary device's passkey
